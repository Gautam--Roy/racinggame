...
const SHIFT_DIP_MS = 180;
const RPM_SMOOTH_RATE = 8; // 1/s, exponential approach rate toward target rpm
 ...
 * UPSHIFT, decaying linearly to 0 over SHIFT_DIP_MS. Downshifts (can happen
 * when slowing down) produce no dip — only upshifts under acceleration jolt
 * the drivetrain.
 *
...
export class GearBox {
  ...
  private static readonly REARM_GUARD_MS = SHIFT_DIP_MS * 2;

  update(speedRatio: number, dtSec: number): GearState {
    const { rpm: targetRpm, gear } = gearRpm(speedRatio);

    if (gear !== this.lastGear) {
      if (gear > this.lastGear && this.msSinceLastArm >= GearBox.REARM_GUARD_MS) {
    ...
    const shiftDip =
      this.msSinceUpshift < SHIFT_DIP_MS ? 1 - this.msSinceUpshift / SHIFT_DIP_MS : 0;

    return { rpm: this.smoothedRpm, gear, shiftDip };
  }
}
